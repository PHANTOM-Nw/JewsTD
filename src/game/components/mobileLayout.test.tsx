...
describe('mobile game layout', () => {
  ...
  it('derives the three-build action deck from the economy config', () => {
    ...
    expect(gameStyles).toMatch(/\.game-phase-hint\s*\{[^}]*position:\s*absolute/s)
    expect(gameStyles).toMatch(/\.game-phase-hint\s*\{[^}]*top:\s*14px/s)
    expect(gameStyles).toMatch(/\.game-phase-hint\s*\{[^}]*pointer-events:\s*none/s)
    expect(gameStyles).toMatch(/\.game-phase-hint\s*\{[^}]*max-width:\s*min\(calc\(100%\s*-\s*28px\),\s*270px\)/s)
