...
当前仓库已形成基础的 12 波可玩原型：每轮必须依次经过 `building → deciding → ready → playing`，放满 3 座随机塔并选择唯一一座保留后才能开波。原型包含 8 种基础宝石、6 种特殊合成塔、动态 BFS 寻路、20 金币清障、最终 Boss 波，以及暂停/继续、胜利/失败和重新开始。游戏采用阳光 Q 版美术、8×10 竖向地图、生成式塔/敌人/障碍精灵和奶油金色触控 UI；Canvas 会在保持 320×400 逻辑分辨率的同时按可用宽度等比缩放。

战斗效果已接入多目标、暴击、溅射、邻近目标穿透、按配置比例减速、毒素和眩晕。自动化测试保护配置、寻路、响应式 Canvas 点击换算、建造/波次状态、矿坑伤害和主要战斗规则；Chromium 视觉与交互回归已覆盖 390×844 的三次建造、三选一、开波、暂停/继续和合成列表，并补充检查 360×800 与短屏 390×693。完整第一波结算以及从第 1 波持续打到第 12 波胜利，尚未在浏览器中完成端到端实测，因此不应把当前验证范围理解为完整通关验收。
...
│   ├── assets/            # Q 版地图、塔、敌人、障碍和场景位图
│   ├── config/            # 地图、塔、敌人和波次配置
│   ├── engine/            # 游戏状态、动作和帧循环
│   ├── pathfinding/       # BFS 寻路及 React 封装
│   ├── rendering/         # Canvas 场景绘制和精灵注册表
│   ├── services/          # 音频等有状态服务
│   ├── types/             # 游戏领域类型
│   └── utils/             # 无状态通用计算
